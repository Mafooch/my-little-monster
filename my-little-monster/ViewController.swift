...
//    setting the alpha when it's dim. the variable used to set the opacity on the skulls
    ...
    var currentItem: UInt32 = 0 // an integer, that can hold more values than a standard int
...
//            use a do block so we have access to the try block which is what we wanna do here since we're not sure these sound resources will work
...
//            saying before we start a timer, let's just stop the old one
    ...
    func changeGameState() {
        ...
        let rand = arc4random_uniform(2) // give me a random number between 0-1
